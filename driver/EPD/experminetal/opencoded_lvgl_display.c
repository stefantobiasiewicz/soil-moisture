...
#ifdef CONFIG_LV_COLOR_DEPTH_8
void lvgl_flush_cb_8bit_epd_opencoded(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
...
void lvgl_set_px_cb_8bit_epd_opencoded(lv_disp_drv_t *disp_drv, uint8_t *buf, lv_coord_t buf_w, lv_coord_t x,
			  lv_coord_t y, lv_color_t color, lv_opa_t opa)
{
	uint8_t *buf_xy = (uint8_t *)(buf + x + y * buf_w);
	if(LV_COLOR_GET_R(color) > 0x01 && (LV_COLOR_GET_G(color) < 0x4) && (LV_COLOR_GET_B(color) < 0x4)) {
        *buf_xy = 0x02;
	} else if((LV_COLOR_GET_R(color) > 0x4) || (LV_COLOR_GET_G(color) > 0x4) || (LV_COLOR_GET_B(color) > 0x2)) {
        *buf_xy = 0x00;
    }else {
		*buf_xy = 0x01;
	}
...
void lvgl_rounder_cb_8bit_epd_opencoded(lv_disp_drv_t *disp_drv, lv_area_t *area)
{
	area->x1 &= ~0x7;
	area->x2 |= 0x7;
}
