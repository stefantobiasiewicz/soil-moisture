...
LOG_MODULE_REGISTER(zigbee_app_utils, LOG_LEVEL_DBG);
...
zb_ret_t zigbee_default_signal_handler(zb_bufid_t bufid)
{
	...
	case ZB_ZDO_SIGNAL_SKIP_STARTUP:
		...
		ZB_ERROR_CHECK(
			...
			zb_enable_panid_conflict_resolution(true);
	...
	case ZB_ZDO_SIGNAL_LEAVE:
		...
		if (status == RET_OK) {
			...
			if (zb_get_network_role() ==
				...
				/**
				 * OP modyfications
				 * Afetr device leave not start rejoining procedure
				 */
				// start_network_rejoin();
			}
		} else {
 ...
 */
void stop_network_rejoin(zb_uint8_t was_scheduled)
{
...
/**
 * https://devzone.nordicsemi.com/f/nordic-q-a/87938/zb_address_short_by_ref-inexplicably-returns-0xffff
 */
